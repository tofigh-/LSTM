...
def train(vanilla_rnn, n_iters, resume=RESUME):
                ...
                if batch_num % 1000 == 0 and train_mode:
                    ...
                    print "{i}ith week Natioanl AVG Train KPI is {t_kpi}".format(
                        ...
                        t_kpi=np.sum(
                            ...
                            np.array(kpi_sale_scale[week_idx])[:, 0:-1]) * 100)
    ...
    for n_iter in range(1, n_iters + 1):
        ...
        if n_iter == change_optimizer_epoch:
            ...
            scheduler_encoder = ReduceLROnPlateau(vanilla_rnn.encoder_optimizer, mode='min', patience=1, factor=0.5)
            scheduler_decoder = ReduceLROnPlateau(vanilla_rnn.future_decoder_optimizer, mode='min', patience=1, factor=0.5)
        vanilla_rnn.mode(train_mode=True)
        _, _, _, \
        train_sale_kpi, \
