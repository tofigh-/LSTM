...
def predict(model, loss_function, loss_function2, targets_future, inputs):
    ...
    cnn_encoded_input = model.cnn_encode(input_encoder, encoded_mask.squeeze())
    ...
    for week_idx in range(OUTPUT_SIZE):
        ...
        features = torch.cat([cnn_encoded_input,output_prefinal.squeeze(), embedded_features, input_decoder[:, week_idx, :]], dim=1)
