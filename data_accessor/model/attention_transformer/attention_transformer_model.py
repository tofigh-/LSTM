...
def make_model(embedding_descriptions, total_input, N_enc=6, N_dec=6,
    ...
    model = EncoderDecoder(
        embeddings=Embeddings(embedding_descriptions, total_input),
        encoder=Encoder(EncoderLayer(d_model, c(multi_head_attn), c(positionwise_feed_forward), dropout_enc), N_enc),
        near_future_decoder=Decoder(DecoderLayer(d_model, c(multi_head_attn), c(multi_head_attn),
                                                 c(positionwise_feed_forward), dropout_dec), N_dec),
        far_future_decoder=Decoder(DecoderLayer(d_model, c(multi_head_attn), c(multi_head_attn),
                                                c(positionwise_feed_forward), dropout_dec), N_dec),
        near_future_generator=GeneratorLayer(d_model * 2 + num_embedded_features, num_output=14),
        far_future_generator=GeneratorLayer(d_model * 2 + num_embedded_features, num_output=14),
        model_size=d_model)
