...
def cache_data(dataset):
    ...
    def save_batches(all_losses, all_batches, batch_num):
        ...
        seed(42)
        shuffle(all_losses)
        output_path_batch = "chuck_saved_{seq_length}_{output_size}_{batch_num}_batch.{extension}".format(
            batch_num=batch_num,
            seq_length=TOTAL_INPUT,
            output_size=OUTPUT_SIZE,
            extension=extension)
        output_path_loss = "chuck_saved_{seq_length}_{output_size}_{batch_num}_loss.{extension}".format(
            batch_num=batch_num,
            seq_length=TOTAL_INPUT,
            output_size=OUTPUT_SIZE,
            extension=extension)
